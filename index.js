...
const styles = {
  ...
  xAxisDark: {
    ...
    position: 0,
    get gridColor() {
      return styles.color.lighterGrey;
    },
    ticks: false,
    //the axis baseline
    get domainColor() {
...
export async function fetchRate() {
  ...
  console.log(json);
  return (
    json.current_rate.cent_equivalent / json.current_rate.hbar_equivalent / 100
  );
}
  ...
  const barGraph = vl
    ...
    .encode(
      ...
      vl
        ...
        .if(hover.empty(true), {
          value:
            style === "dark" ? styles.color.lightGreen : styles.color.darkGreen,
        })
        .value(
          style === "dark" ? styles.color.darkGreen : styles.color.lightGreen
        )
    );
  ...
  const line = vl
    ...
    .transform(
      cumulative
        ? [
            vl
              .window(vl.sum("total").as("cumulative"))
              .sort(vl.field("end_date")),
          ]
        : []
    )
    .title({
